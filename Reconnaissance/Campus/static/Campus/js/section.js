$(document).ready(function () {
    ...
    //delete section
    $('#wrapper').on('click', '.deletesection', function () {
        var id = $(this).attr('data-id');
        var name = $(this).attr('data-name');
        $.confirm({
            title: 'Sure!',
            content: 'You want to delete section "' + name + '"',
            type: 'dark',
            typeAnimated: true,
            buttons: {
                tryAgain: {
                    ...
                    action: function () {
                        $.ajax({
                            ...
                            url: 'ajax/delete',
                            data: {
                                'id': id,'type':'section', 'csrfmiddlewaretoken': $('input[name="csrfmiddlewaretoken"]').val()
                            },
                            dataType: 'json',
                            success: function (data) {
                                if (data.flag == 0) {
