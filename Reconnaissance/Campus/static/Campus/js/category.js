$(document).ready(function() {
    ...
    //delete category
    $('#wrapper').on('click','.deletecategory', function(){
     var id = $(this).attr('data-id');
     var name = $(this).attr('data-name');
     $.confirm({
        title: 'Sure!',
        content: 'You want to delete category "'+name+'"',
        type: 'dark',
        typeAnimated: true,
        buttons: {
            tryAgain: {
                ...
                action: function(){
                      $.ajax({
                        ...
                        url: 'ajax/delete',
                        data: {
                           'id': id,'type':'category' ,'csrfmiddlewaretoken' : $('input[name="csrfmiddlewaretoken"]').val()
                        },
                        dataType: 'json',
                        success: function (data) {
                            if(data.flag == 0){
