$(document).ready(function () {
    ...
    //delete question
    $('#wrapper').on('click', '.deletequestion', function () {
        var id = $(this).attr('data-id');
        var name = $(this).attr('data-name');
        $.confirm({
            title: 'Sure!',
            content: 'You want to delete question "' + name + '"',
            type: 'dark',
            typeAnimated: true,
            buttons: {
                tryAgain: {
                    ...
                    action: function () {
                        $.ajax({
                            ...
                            url: 'ajax/delete',
                            data: {
                                'id': id,'type':'question', 'csrfmiddlewaretoken': $('input[name="csrfmiddlewaretoken"]').val()
                            },
                            dataType: 'json',
                            success: function (data) {
                                if (data.flag == 0) {
